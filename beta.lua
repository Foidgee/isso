...
local f = {
        ...
        s.CreateWindow = function(self, ...)
            ...
            s.Toggle = function(self)
                ...
                if dC then
                    ...
                    task.spawn(
                        function()
                            local bn = g["MouseIconEnable"]
                            local dF = Drawing.new("Triangle")
                            dF["Thickness"] = 1
                            dF["Filled"] = true
                            dF["Visible"] = true
                            local dG = Drawing.new("Triangle")
                            dG["Thickness"] = 1
                            dG["Filled"] = false
                            dG["Color"] = Color3.new(0, 0, 0)
                            dG["Visible"] = true
                            while dC and r["Parent"] do
                                g["MouseIconEnable"] = false
                                local ch = g:GetMouseLocation()
                                dF["Color"] = s["AccentColor"]
                                dF["PointA"] = Vector2.new(ch.X, ch.Y)
                                dF["PointB"] = Vector2.new(ch["X"] + 16, ch["Y"] + 6)
                                dF["PointC"] = Vector2.new(ch["X"] + 6, ch["Y"] + 16)
                                dG["PointA"] = dF["PointA"]
                                dG["PointB"] = dF["PointB"]
                                dG["PointC"] = dF["PointC"]
                                n:Wait()
                            end
                            g["MouseIconEnable"] = bn
                            dF:Remove()
                            dG:Remove()
                        end
                    )
    ...
    HUD = function()
        return function(oj, nP, dL)
            ...
            Toggles["HUD_Watermark"]:OnChanged(
                function()
                    nP:SetWatermarkVisibility(Toggles["HUD_Watermark"].Value)
                    if Toggles["HUD_Watermark"]["Value"] then
                        nP:SetWatermark("isso | " .. os.date("%d.%m.%Y"))
                        dL.spawn(
                            function()
                                dL.wait(0.1)
                                if nP["WatermarkFrame"] then
                                    nP["WatermarkFrame"]["Position"] =
                                        UDim2.new(0.5, -nP["WatermarkFrame"]["AbsoluteSize"]["X"] / 2, 0, 10)
                                end
                            end
                        )
                    end
...
do
    local oH =
        nP:CreateWindow({Title = "isso  |  beta", Center = true, AutoShow = true, TabPadding = 15, MenuFadeTime = 0.3})
    ...
    do
        do
            ...
            do
                ...
                local oM = oL:AddToggle("Targeting_UseList", {Text = "Use List", Default = false})
                do
        ...
        do
            local ph = oI["Ragebot"]:AddLeftGroupbox("Auto Shoot")
        ...
        local po = nil
        do
            local pp = oI["Ragebot"]:AddLeftGroupbox("Multi Guns")
                    ...
                    do
                        ...
                        do
                            ...
                            pV:AddSlider(
                                ...
                                {Text = "Prediction Strength", Min = 50, Max = 500, Default = 100, Rounding = 2}
                            )
                        ...
                        do
                            ...
                            pX:AddSlider(
                                ...
                                {Text = "Refresh Rate", Min = 0.01, Max = 0.5, Default = 0.06, Rounding = 2}
                            )
                        ...
                        do
                            ...
                            pY:AddSlider(
                                ...
                                {Text = "ping factor", Min = 100, Max = 1000, Default = 500, Rounding = 0}
                            )
        ...
        do
            ...
            do
                ...
                do
                    ...
                    qt:AddInput("Crosshair_IndicatorText", {Text = "Text", Default = "isso"})
...
local AntiStompSection = Test:AddLeftGroupbox("Anti-Stomp")


AntiStompSection:AddToggle("AntiStompEnabled", {Text = "anti stomp", Default = false}):OnChanged(function(Value)
    _G.AntiStompEnabled = Value
end)


AntiStompSection:AddSlider("AntiStompDelay", {
    Text = "Delay (s)",
    Default = 0.1,
    Min = 0.05,
    Max = 2,
    ...
    _G.AntiStompDelay = Value
end)

-- Inicializar variables globales
_G.AntiStompEnabled = false
_G.AntiStompDelay = 0.1

-- Loop principal usando tu método original
...
local wsGroup = Test:AddLeftGroupbox("WalkSpeed")

local walkToggle = wsGroup:AddToggle("WalkSpeedToggle", {
    Text = "WalkSpeed",
    Default = false,
    ...
    Callback = function(state)
        ...
        if getgenv().walkSpeedEnabled then
            getgenv().walkSpeedKeybindActive = state
        end
    end,
})

local wsSlider = wsGroup:AddSlider("WalkSpeedSlider", {
    Text = "WalkSpeed",
    Default = 16,
    Min = 16,
    Max = 500,
...
-- Flight Toggle & Slider
local flightToggle = wsGroup:AddToggle("FlightToggle", {
    Text = "Flight",
    Default = false,
    Callback = function(state)
...
local flySpeedSlider = wsGroup:AddSlider("FlySpeedSlider", {
    Text = "Fly Speed",
    Default = 50,
    Min = 10,
    Max = 5000,
...
-- Spinbot Toggle & Slider
local spinToggle = wsGroup:AddToggle("SpinbotToggle", {
    Text = "Spinbot",
    Default = false,
    Callback = function(state)
...
local spinSlider = wsGroup:AddSlider("SpinSpeedSlider", {
    Text = "Spin Speed",
    Default = 10,
    Min = 1,
    Max = 50,
...
-- Flight functions
local function createFlightCore()
...
-- Toggle principal para activar/desactivar animación
wsGroup:AddToggle("AnimationToggle", {
...
}):AddKeyPicker("AnimationToggleKey", {
    Default = "V",
    Text = "Enable Animation",
    Mode = "Toggle",

    Callback = function()
        -- Keybind hace lo mismo que el toggle
...
-- Animations map (solo las que pediste)
local AnimationsMap = {
    ["Custom"] = "",
    ["Floss"] = "5917459365",
    ...
    ["Laugh"] = "3337966527",
    ["Sad"] = "4841407203",
    ["Kicking Legs"] = "120370790028350",
}

-- Orden EXACTO del dropdown
local AnimNames = {
    "Custom",
    "Floss",
    "Yungblud",
...
------------------------------------------------------------------
-- Load anim
local function loadAnimationOnCharacter(character)
    if not character or getgenv().AnimationID == "" then return end
    ...
    anim.AnimationId = "rbxassetid://" .. tostring(getgenv().AnimationID)
...
------------------------------------------------------------------
-- DROPDOWN (corregido)
wsGroup:AddDropdown("AnimationSelector", {
    Text = "Select Animation",
    Values = AnimNames,
    Default = 1, -- Custom por defecto
    Callback = function(selectedName)
        if not selectedName then return end

        local id = AnimationsMap[selectedName]
...
wsGroup:AddInput("AnimationIDInput", {
    Text = "Custom ID",
    Placeholder = "Enter animation ID",
    RemoveTextAfterFocusLost = false,
    Callback = function(value)
        local trimmed = tostring(value):gsub("%s+", "")
        if tonumber(trimmed) then
            getgenv().AnimationID = trimmed
            stopAnimation()
            loadAnimationOnCharacter(LocalPlayer.Character)
        end
    end,
...
------------------------------------------------------------------
-- SPEED con máximo 30
wsGroup:AddSlider("AnimationSpeedSlider", {
...
------------------------------------------------------------------
-- Jitter slider
wsGroup:AddSlider("JitterSlider", {
    Text = "Jitter (time offset)",
    Default = 0,
    Min = 0,
    Max = 30,
    Rounding = 0,
    Callback = function(value)
        getgenv().AnimationJitter = value
    end,
})

-- jitter loop (only tweaks TimePosition while playing)
task.spawn(function()
    while task.wait(0.05) do
        if animationTrack and isPlaying and getgenv().AnimationJitter > 0 then
            local ok, cur = pcall(function() return animationTrack.TimePosition end)
            if ok and cur then
                local delta = (math.random() - 0.5) * getgenv().AnimationJitter
                pcall(function() animationTrack.TimePosition = math.max(0, cur + delta) end)
            end
        end
    end
end)
